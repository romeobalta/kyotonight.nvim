...
local Groups = require("kyotonight.groups")
...
function M.extras()
  local Extra = require("kyotonight.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/kyotonight/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/kyotonight/config.lua", "\n(--@class kyotonight%.Config.-\n})")
